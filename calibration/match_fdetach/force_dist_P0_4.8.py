...
detachment_force = 4.0  # read off roughly from the plot; use `pyafv.calibrate.auto_calibrate` to get a more precise value
delta = target_delta(phys, detachment_force) # 0.185
...
mask = (data[:,1] > -5)
ax.plot(centroid_distances[mask][:-40:5*step], data[mask,1][:-40:5*step], 's', color="C4", markerfacecolor="None", label='FV', zorder=0, clip_on=False)
ax.plot(centroid_distances[mask][-40::step], data[mask,1][-40::step], 's', color="C4", markerfacecolor="None", zorder=0, clip_on=False)


#====================================================
# Deformable polygon results

data = np.load('../fix_delta/force_dist_P0_4.8.npy')
ext_forces = data[:, 1]
...
ax.set_xticks([1.0, 1.5, 2*l, 2.0])
ax.set_xticklabels([r'$1.0$', r'$1.5$', r'$2\ell_0$', r'$2.0$'])

ax.set_xlabel(r'Centroid distance')
ax.set_ylabel(r'Force $f$')

ax.set_title(r'$P_0=4.8$')

ax.set_ylim(-5, 8)
ax.set_xlim(0.9, 2.2)
ax.legend(frameon=False, loc='upper left')

plt.savefig("force_dist_P0_4.8.png", dpi=150, bbox_inches='tight')
